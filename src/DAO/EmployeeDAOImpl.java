...
public class EmployeeDAOImpl implements EmployeeDAOI {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(SQL)) {
            try (ResultSet rset = stmt.executeQuery()) {
                while (rset.next()) {
                    ...
                    employees.add(new Employee(id, nom, prenom, salaire,
                            email, phone, Role.valueOf(role), Poste.valueOf(poste)));
                }
            }
    ...
    public void ajouter(Employee employee) {
        String SQL = "INSERT INTO employee (nom, prenom, salaire, email, phone, role, poste)" +
                " VALUES (?, ?, ?, ?, ?, ?, ?)";
        EmployeeController.viderLesChamps();
        try (PreparedStatement stmt = connection.prepareStatement(SQL)) {
            ...
            try (ResultSet rset = stmt.executeQuery()) {
                if(rset.next()) {
                    employee = new Employee(rset.getInt("id"), rset.getString("nom")
                            , rset.getString("prenom"), rset.getDouble("salaire"),
                            rset.getString("email"), rset.getString("phone"),
                            Role.valueOf(rset.getString("role")),
                            Poste.valueOf(rset.getString("poste")));
                }
            }catch(SQLException e) {
                e.printStackTrace();
            }
    ...
    public void modifier(Employee employee, int EmployeeId) {
        String SQL = "UPDATE employee SET nom = ?, prenom = ?, salaire = ?," +
                " email = ?, phone = ?, role = ?, poste = ? WHERE id = ?";
        EmployeeController.viderLesChamps();
        try (PreparedStatement stmt = connection.prepareStatement(SQL)) {
            stmt.setString(1, employee.getNom());
            stmt.setString(2, employee.getPrenom());
